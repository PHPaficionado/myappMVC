                        ...
                        switch ($rule) {
                            case 'min':
                                if(strlen($value) < $rule_value) {
                                    $this->addError(["{$display} must be a minimun of {$rule_value} charaters.", $item]);
                                }
                            break;  

                            case 'max':
                                if(strlen($value) > $rule_value) {
                                    $this->addError(["{$display} must be a maximum of {$rule_value} charaters.", $item]);
                                }
                            break; 
                            ...
                            case 'is_numeric':
                                if(!is_numeric($value)) {
                                    $this->addError(["{$display} has to be a number. Please use a numeric value {$display}", $item]);
                                }
                            break;
        ...
        public function displayErrors()
        {
            ...
            foreach($this->_errors as $error) {
                if(is_array($error)) {
                    ...
                    $html .= '<script>jQuery("document").ready(function(){jQuery("#'.$error[1].'").parent().closest("div").addClass("has-danger");});</script>';
                } else {
